...
def getVaderLines():
	...
	delimiter = " +++$+++ "
	inFile = open("star-wars-movie-lines_600.txt")
	lines = inFile.readlines()
	inFile.close()
	outFile = open("star-wars-movie-vader-lines_600.txt", "w")
	for i,line in enumerate(lines[:-1]):
		#if i%260 == 0:
		#	print(line.split(delimiter)[-1])
	#	if line.split(delimiter)[0] == "L702125":
#			print(i)
			#break
		
		nextLine = lines[i+1]
		if nextLine.split(delimiter)[3] == "VADER":
			outFile.write(line)
...
def get_225():
	...
	delimiter = " +++$+++ "
	inFile = open("star-wars-movie-vader-lines_600.txt")
	lines = inFile.readlines()
	inFile.close()
	outFile = open("star-wars-movie-vader-lines_225.txt", "w")
...
def getVaderConvos():
	...
	delimiter = " +++$+++ "
	inFile = open("star-wars-movie-vader-lines_225.txt")
	lines = inFile.readlines()
	inFile.close()
	outFile = open("star-wars-vader-only-conversations_225.txt", "w")
	for i, line in enumerate(lines[:-1]):
		#if i == 58500:
		#	break
		if i%2 == 0:
			curLine = line.split(delimiter)
			nextLine = lines[i+1].split(delimiter)
			char1 = line.split(delimiter)[1]
			char2 = lines[i+1].split(delimiter)[1]
			outputString = char1 + delimiter + char2 + delimiter + "m617" + delimiter + str([curLine[0], nextLine[0]])
